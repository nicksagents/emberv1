...
export function buildSharedPrompt(settings: Settings, role: Role, options: SharedPromptOptions = {}): string {
  if (options.compact) {
    return [
      ...
      "- Workspace root is the default project, not a sandbox. Active file, terminal, browser, and desktop tools can act on the host machine.",
  ...
  return [
    ...
    "",
    "## Host Access",
    "The workspace root is Ember's default project, not a hard sandbox boundary for the native tools.",
    "When filesystem tools are active, they can use absolute host paths such as /Users/nick/Desktop or ~/Downloads.",
    "When terminal tools are active, they can run host-machine shell commands.",
    "When browser or desktop tools are active, they can drive those surfaces on the local machine.",
    "",
